<script setup>
  ...
  const handleSubmit = async () => {
    ...
    // router.unshift(`/jobs/${response.data.id}`)
    router.push(`/jobs/${response.data.id}`)
  } catch (error) {
    console.log('Error fetching jobs');
    toast.error('Update was not added')
  } 
  }
        ...
        >
          <form @submit.prevent="handleSubmit">
            ...
            <div>
              ...
              <button
                class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 mt-2 rounded-full w-full focus:outline-none focus:shadow-outline"
                @click="routeBack">
                <h3>Cancel</h3>
              </button>
            </div>
